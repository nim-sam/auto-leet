...
*/
fn flatten_submission(path string, sub Submission) (string, string, string) {
...
fn update_tmp(token string, session string, path string) {
    ...
    submissions := get_accepted_submissions(token, session)
    ...
    for sub in submissions {
		...
		if !(sub.title_slug in readme_complete_slugs) {
			...
			mut readme := get_problem_readme(token, session, sub.title_slug)
...
*/
fn sync_submissions(token string, session string, repo_url string, pat string) {
